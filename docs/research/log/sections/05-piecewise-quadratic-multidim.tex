...
\[
  ...
  \left\{
    F:
    F \text{ is a connected }(d-1)\text{-dimensional face of }
    K_i\cap K_j
  \right\}.
\]

\begin{sidenote}
Each cell $K_i$ carries its own quadratic piece $V_i$; the function need not be
given by one quadratic expression on $K_i\cup K_j$. Two such cells can touch
along more than one $(d-1)$-dimensional interface, or facet. The set
$\mathcal F_{ij}$ records these connected faces separately so that the normal
and the Tanaka condition can be specified on each one.
\end{sidenote}

For each $F\in\mathcal F_{ij}$, choose a unit normal
$n_{i\to j,F}$ pointing from $K_i$ into $K_j$. If
\[
  F\subseteq \{x:h_F^\top x=\gamma_F\},
\]
then one may take
\[
  n_{i\to j,F}
  =
  \operatorname{sign}\!\left(h_F^\top(x_j-x_i)\right)
  \frac{h_F}{\|h_F\|},
\]
where $x_i\in\operatorname{int}(K_i)$ and
$x_j\in\operatorname{int}(K_j)$.
...
\frac{1}{2}
\sum_{i<j}
\sum_{F\in\mathcal F_{ij}}
\int_0^{t \wedge \tau_K}
\left(
\nabla V_j(X_s) - \nabla V_i(X_s)
\right)\cdot n_{i\to j,F}\,
dL_s^F .
\end{align}

Here $L_t^F$ denotes the surface local time accumulated by $X_t$ on the face
$F$.
...
\item
The interface contribution is nonpositive. The Tanaka interface condition is
imposed face by face: for every $F\in\mathcal F_{ij}$, we assume
\[
    \left(
        \nabla V_j(x) - \nabla V_i(x)
    \right)\cdot n_{i\to j,F}
...
\[
    ...
    \qquad \text{on } F,
\]
for every $F\in\mathcal F_{ij}$, and $L_t^F$ is nondecreasing in $t$, then the
interface term $K_t$ is
a supermartingale.
...
\[
    ...
    \qquad F\in\mathcal F_{ij}.
\]
Also, $dL_s^F$ is a nonnegative measure, because surface local time is
nondecreasing. Therefore
\[
    \int_r^t \gamma_{ij,F}(X_s)dL_s^F \leq 0.
\]
...
\[
\nabla V_i(x)\cdot n_{i\to j,F},
\]
while the normal derivative from the $K_j$ side is
\[
\nabla V_j(x)\cdot n_{i\to j,F}.
\]
...
\subsection{Reach--avoid stopping convention}

The exit-time result above is a general auxiliary result.  For the
reach--avoid verifier, we stop earlier.  Let $X_\tau\subseteq K$ be the target
and let $\beta$ be the upper certificate level.  The stopping time used by the
verifier is
\[
\tau
=
...
\right\}.
\]
Thus touching the target, the boundary of the verification domain, or the
super-$\beta$ set is the stopping moment.  In particular, the unsafe set is
not a separate stopping event: the condition $V\geq\beta$ on the unsafe set
ensures that a continuous path reaching it has already reached the
super-$\beta$ stopping set.

With this convention, the interior generator condition is needed only on
\[
\left\{x\in\operatorname{int}(K)\setminus X_\tau:V(x)<\beta\right\}.
\]
For verification it is conservative to check its closure, replacing
$V<\beta$ by $V\leq\beta$.  Likewise, the normal-derivative jump condition is
needed only on the portions of internal faces outside the target and in this
sub-$\beta$ basin.  No generator or interface condition is required after the
process has reached any of the three stopping sets.

The stopping convention also requires the domain-boundary separation
condition
\[
\inf_{x\in\partial K\setminus X_\tau}V(x)\geq\beta.
\]
Together with
\[
\sup_{x\in X_0}V(x)\leq\alpha,
\qquad
\inf_{x\in X_u}V(x)\geq\beta,
\qquad
\alpha<\beta,
\]
these are the conditions implemented by the multidimensional
piecewise-quadratic verifier.  Applying the preceding It\^o--Tanaka argument
up to $\tau$ shows that $V(X_{t\wedge\tau})$ is a supermartingale.

\subsection{Interpreting the probability bound in the OU benchmark}

The failure event in this stopping convention is
\[
  \{\tau_{X_u}\wedge\tau_{\partial K}<\tau_{X_\tau}\},
\]
namely, reaching the unsafe set \emph{or leaving the certified domain} before
reaching the target.  The boundary condition
$V\geq\beta$ on $(X_u\cup\partial K)\setminus X_\tau$, together with
$\sup_{X_0}V\leq\alpha$, gives the uniform bound
\[
  \sup_{x\in X_0}
  \mathbb P_x\!\left(
    \tau_{X_u}\wedge\tau_{\partial K}<\tau_{X_\tau}
  ...
  \leq \frac{\alpha}{\beta}.
\]
It is therefore incorrect to read $\alpha/\beta$ as bounding unsafe hits
alone while treating domain exit as a neutral outcome.

This distinction is visible in the two-dimensional OU experiment with the
enlarged target $[-0.5,0.5]^2$ and $\beta=2$.  The finite-difference
committor solving $\mathcal LV=0$, with value zero on the target and $\beta$
on the unsafe and domain boundaries, gives composite failure probabilities
$V/\beta$ between $0.241$ and $0.555$ on $X_0$; their average under a uniform
initial state is $0.370$.  Thus the smallest committor-derived threshold is
approximately $\alpha=1.11$, and $\alpha/\beta\approx0.555$ is the
\emph{worst-case} probability over $X_0$, not a prediction for a uniformly
sampled initial state.

As a numerical cross-check, $100{,}000$ exact-transition OU trajectories from
a uniform initial state, inspected on a time grid of width $5\times10^{-4}$,
gave target, unsafe, and domain-exit frequencies of $64.15\%$, $10.70\%$, and
$25.15\%$, respectively.  The observed composite failure frequency was
$35.85\%$, close to the finite-difference uniform average; the remaining
difference is consistent with discretization in both calculations.  A
separate Dirichlet solve for unsafe-before-target-or-exit gave an average of
$10.4\%$ and a worst case of $13.8\%$ on $X_0$.  The apparent gap between
$55.5\%$ and roughly $36\%$ is therefore mainly a worst-case-versus-average
comparison, while the larger gap to roughly $11\%$ additionally omits domain
exit from the failure event.

\subsection{Asymptotic completeness of piecewise quadratic certificates}

The theorem below retains the stronger exit-time formulation: it assumes the
generator inequality throughout $K$ and therefore proves a supermartingale up
to $\tau_K$.  To use its approximants as reach--avoid certificates under the
convention above, one additionally needs a strict boundary margin for the
smooth certificate on $\partial K\setminus X_\tau$; uniform approximation then
preserves $V_h\geq\beta$ there.
