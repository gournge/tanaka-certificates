...
\begin{enumerate}
  ...
  \begin{equation}
    ...
    \inf_{x\in X_u}V(x)\geq\beta \qquad\text{and}\qquad
    \inf_{x\in\partial D\setminus X_\tau}V(x)\geq\beta
  \end{equation}
...
Following the piecewise-quadratic setup, suppose that the compact verification
region $K=D$ is partitioned into finitely many polyhedral cells,
\[
  K=\bigcup_{i=1}^N K_i,
\]
...
\begin{equation}
  ...
  =\inf\left\{t\geq0:
    ...
    V(X_t)\geq\beta
  \right\}.
  \label{eq:reach-avoid-stopping-time}
\end{equation}
...
$V(X_{t\wedge\tau})$ is a supermartingale.  The generator and interface
conditions are required only before $\tau$, namely outside the target and in
the sub-$\beta$ basin.
...
The process is stopped upon touching the target, the boundary of the domain,
or the super-$\beta$ set.  The unsafe-set condition guarantees that a
continuous path reaching the unsafe set has reached the super-$\beta$ set no
...
\begin{figure}[h]
  ...
  $X_t$ and a stopped certificate process.  This example illustrates the
  interior and interface checks but, as discussed below, does not satisfy the
  domain-boundary condition of the full reach--avoid verifier.}
...
\subsection{Initial, unsafe, and domain-boundary conditions}
...
\[
  ...
  \beta.
\]
Therefore the initial/unsafe separation condition holds.  It is important,
however, that this candidate does \emph{not} satisfy the domain-boundary
condition required by the stopping convention
\eqref{eq:reach-avoid-stopping-time}.  For example, on the horizontal domain
edges,
\[
  V(0,\pm1)=0<\beta=\frac14.
\]
Unless these boundary points are included in the target, the exact verifier
must reject this candidate.  The example below should therefore be read as an
explicit demonstration of the generator and interface local-time conditions,
not as a complete reach--avoid certificate.
...
The following structural conditions from the multidimensional
piecewise-quadratic argument hold:
\begin{enumerate}
  \item $V$ is continuous and piecewise quadratic on the polyhedral partition
  $K=K_1\cup K_2$,
  \item the initial/unsafe separation inequalities hold with
...
\end{enumerate}
Consequently, the example establishes the intended drift and local-time
behavior up to the stopping time, but it is not a complete reach--avoid
verification example because the domain-boundary separation condition fails.
It remains a fully explicit example where the curvature term in the quadratic
pieces compensates for the positive drift in the $x_1$ direction.
...
\begin{enumerate}
  ...
  This historical experiment did not yet penalize points on the boundary of
  the training domain.  Under the stopping convention
  \eqref{eq:reach-avoid-stopping-time}, non-target domain-boundary samples also
  require the lower-level penalty
  $\max\{(\beta+\delta)-V(x),0\}$.
